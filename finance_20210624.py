...
# dateval = date.today()
dateval = '2021/03/24'
if isinstance(dateval, date) == False:
    dateval = dateval.replace("-","").replace("/","")
    dateval = date(int(str(dateval)[0:4]), int(str(dateval)[4:6]), int(str(dateval)[6:8]))

quarter = (8 - 4) % 4 
# quarter = int((dateval.month - 1) / 3 + 1)
# result = date(dateval.year, 3 * quarter - 2, 1)
# type(date.today())
...
def getYearQuarterVal(cfgfile, dateval, num):
    ...
    q_cal = int((dateval.month - 1) / 3 + 1) + int(num)

    if q_cal in range(1,5):
        return str(dateval.year), str(q_cal)
    else:
    ...
    # PreviousQuarter = DateQuarter.from_date(date.today()) - 1
    # if y == None:
...
def getQuarterFirstDate(tbobj):
    ...
    return date(int(show_date[0]), int(show_date[1]) - 2, 1)
...
year, quarter = getYearQuarterVal(cfg_fname, date.today(), -1)
