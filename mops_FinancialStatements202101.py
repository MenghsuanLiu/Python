...
for CompanyID in StockList:
    ...
    for GL in ("1XXX", "2XXX", "3110"):        
        ...
        if record == 0:            
            head_val = tb_BalanceSheet.find("td", text = GL).find_parent("tr").find("span", class_ ="zh").text.strip().replace("（","(").replace("）",")")
    ...
    for GL in ("4000", "5950", "6000", "6500", "6900", "7000", "8610", "7950", "8200", "9850", "6300"):
        ...
        if record == 0:
            try:
                head_val = tb_ComprehensiveIncome.find("td", text = GL).find_parent("tr").find("span", class_ ="zh").text.strip().replace("（","(").replace("）",")")
            except:
                head_val = GL 
            data_head.append(head_val)
    ...
    for GL in ("A10000", "A20900", "A20100", "A20200"):
        ...
        # 抓第二個column,中文當成Header(只在第一個loop抓)
        if record == 0:
            head_val = tb_CashFlow.find("td", text = GL).find_parent("tr").find("span", class_ ="zh").text.strip().replace("（","(").replace("）",")")
            data_head.append(head_val)
    record += 1
