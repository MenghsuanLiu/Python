...
def splitPSMCRevenueByBU(list):
    ...
    try:
        ...
        ym = list[0]
        ym = ym.split("-")[0] + str(ym.split("-")[1] if int(ym.split("-")
                                                            [1]) >= 10 else "0" + ym.split("-")[1]) + "%"
        with pymssql.connect(server="8AEISS01", user="sap_user", password=pwd, database="BIDC") as conn:
            with conn.cursor() as cursor:
                try:
                    ...
                    revenueval = round([float(r[0])
                                       for r in cursor.fetchall()][0] / 1000, 0)
                    return revenueval
                except:
    ...
    try:
        with pymssql.connect(server="RAOICD01", user="owner_sap", password=dectry(pwd_enc), database="BIDC") as conn:
            with conn.cursor() as cursor:
                try:
                    cursor.execute(
                        "SELECT StockID, StockName, Market, Industry, EnShowName FROM BIDC.dbo.mopsStockCompanyInfo")
...
def updateRevenue_mssql(DataI, ymclist, cfg):
    ...
    with pymssql.connect(server="RAOICD01", user="owner_sap", password=dectry(pwd_enc), database="BIDC") as conn:
        with conn.cursor() as cursor:
            ...
            try:
                cursor.executemany(
                    "DELETE FROM BIDC.dbo.mopsRevenueByCompany WHERE YearMonth = %s", ym_tuple)
                conn.commit()
            ...
            try:
                cursor.executemany(
                    "INSERT INTO BIDC.dbo.mopsRevenueByCompany (YearMonth, BU, StockID, Revenue, Remark) VALUES (%s, %s, %s, %d, %s)", item_tuple)
                conn.commit()
...
def updateCompList_mssql(DataI, cfg):
    ...
    with pymssql.connect(server="RAOICD01", user="owner_sap", password=dectry(pwd_enc), database="BIDC") as conn:
        with conn.cursor() as cursor:
            ...
            try:
                cursor.executemany(
                    "DELETE FROM BIDC.dbo.mopsStockCompanyInfo WHERE StockID = %s", sid_tuple)
                conn.commit()
            ...
            try:
                cursor.executemany(
                    "INSERT INTO BIDC.dbo.mopsStockCompanyInfo (StockID, StockName, Market, Industry, EnShowName) VALUES (%s, %s, %s, %s, %s)", comp_tuple)
                conn.commit()
...
def writeExcel(DataH, DataI, ymlist, cfgfile):
    ...
    try:
        df_imcome.to_excel(
            rf"{file_path}/Revenue_{fname_ym}.xlsx", index=False)
...
stockcatg = getConfigData(cfg_fname, "stocktype")
# 取得需要抓取的年月清單
    ...
    for period in ym_clist:
        ...
        for ind in industy:
            ...
            for rows in TB_Obj.select("table > tr")[2:]:
                # 股票代碼(空值換下一筆)
                StockID = getTBColval(rows, 1) if getTBColval(
                    rows, 1) != "" else None
                if StockID == None:
                    continue
                else:
                ...
                for i in range(2, 12):
                    ...
                    if i in [3, 4, 5, 8, 9]:
                        itemlist.append(
                            int(getTBColval(rows, i) if getTBColval(rows, i) != "" else 0))
                    # [LastPercent, YoYPercent, DiffPercent]
                    if i in [6, 7, 10]:
                        itemlist.append(
                            float(getTBColval(rows, i) if getTBColval(rows, i) != "" else 0))
                # [市場, 產業]
                for l in [market, ind]:
                    itemlist.append(l)
                ...
                if PSMC_Revenue_L == 0:
                    itemlist.append("")
                    itemlist.append(
                        int(getTBColval(rows, 3) if getTBColval(rows, 3) != "" else 0) * 1000)
                else:
